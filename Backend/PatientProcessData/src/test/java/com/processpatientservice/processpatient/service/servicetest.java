...
@ContextConfiguration(classes = ProcessService.class)
...
void getpatienttest(){
		...
		assertEquals(patientot.getPatientName(),processService.getPatientDetails(id).getBody().getPatientName());
	}
	...
	void invalidpatienttest() {
		...
		when(patientRepo.findById(id)).thenReturn(Optional.of(patientot));
		assertEquals(null,processService.getPatientDetails(id).getBody().getPatientName());
	}
	...
	void updatetest() {
		String id="Arjun";
		 ...
		 assertEquals(new ResponseEntity<>(HttpStatus.OK),processService.processPatientDetails(patienrequest, id));
